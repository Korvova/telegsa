...
export default function useTonPayment(chatId: string) {
  ...
  async function loadWalletStatus() {
    const st = await fetch(`/api/wallet/ton/status?chatId=${encodeURIComponent(chatId)}`);
  ...
  async function startPayment(amountTon: number, amountRub: number | null, taskId?: string | null) {
    ...
    const fr = await fetch('/api/bounty/fund-request', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ chatId, ownerAddress, amount: amountTon, taskId: taskId || null }) });
    ...
    try { await fetch('/api/bounty/draft/set', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ chatId, amountTon, amountRub: amountRub ?? undefined }) }); } catch {}
  }

  async function refund(amountTon: number) {
    const sj = await loadWalletStatus();
    const ownerAddress = sj?.address || '';
    const rr = await fetch('/api/bounty/refund-request', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ chatId, ownerAddress, amount: amountTon }) });
    const rj = await rr.json().catch(()=>({ ok:false, error:'internal' }));
    if (!rr.ok || !rj?.ok) throw new Error(String(rj?.error || `http_${rr.status}`));
    try { await fetch('/api/bounty/draft/clear', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ chatId }) }); } catch {}
  }

  async function loadDraft() {
    try {
      const r = await fetch(`/api/bounty/draft/get?chatId=${encodeURIComponent(chatId)}`);
