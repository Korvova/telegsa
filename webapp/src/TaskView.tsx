...
import { getTask, updateTask, completeTask } from './api';

export default function TaskView({ taskId, onClose, onChanged }: {
  taskId: string;
  onClose: () => void;
  onChanged?: () => void;
}) {
  ...
  useEffect(() => {
    setLoading(true);
    getTask(taskId)
      .then((r) => { setTask(r.task); setText(r.task.text); })
    ...
    <div style={{ minHeight: '100vh', background: '#0f1216', color: '#e8eaed', padding: 16 }}>
      ...
      <div style={{ background: '#1b2030', border: '1px solid #2a3346', borderRadius: 16, padding: 16 }}>
        ...
        <textarea
          ...
          rows={6}
          style={{
            width: '100%', background: '#121722', color: '#e8eaed',
            border: '1px solid #2a3346', borderRadius: 12, padding: 12, resize: 'vertical'
          }}
        />
        <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
          <button onClick={save} disabled={saving || !text.trim()}
            style={{ padding: '10px 14px', borderRadius: 12, border: '1px solid #2a3346', background: '#202840', color: '#e8eaed', cursor: 'pointer' }}>
            Сохранить
          </button>
          <button onClick={complete} disabled={saving}
            style={{ padding: '10px 14px', borderRadius: 12, border: '1px solid #2a3346', background: '#234324', color: '#d7ffd7', cursor: 'pointer' }}>
            Завершить → Done
          </button>
        </div>
      </div>
